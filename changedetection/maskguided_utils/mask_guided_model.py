...
class SSM_head(nn.Module):
    def __init__(self, mode='dilation', dim=128, num_heads=8, ffn_expansion_factor=4, bias=False,
                 LayerNorm_type='WithBias'):
        super(SSM_head, self).__init__()
        self.norm1 = LayerNorm(dim, LayerNorm_type)
        self.attn = Attention(dim, num_heads, bias, mode)
        self.norm2 = LayerNorm(dim, LayerNorm_type)
        self.ffn = FeedForward(dim, ffn_expansion_factor, bias)
...
class MSASCD_module(nn.Module):
    def __init__(self, dim=128):
        super(MSASCD_module, self).__init__()
        self.F_TA = MSA_head()
        self.Fuse = nn.Conv2d(dim, dim, kernel_size=3, padding=1)
        self.Fuse2 = nn.Sequential(nn.Conv2d(dim, dim, kernel_size=1), nn.Conv2d(dim, dim, kernel_size=3, padding=1),
                                   nn.BatchNorm2d(dim), nn.ReLU(inplace=True))
...
class MSASCD_background_module(nn.Module):
    def __init__(self, dim=128):
        super(MSASCD_background_module, self).__init__()
        self.F_TA = MSA_head()
        self.Fuse = nn.Conv2d(dim, dim, kernel_size=3, padding=1)
        self.Fuse2 = nn.Sequential(nn.Conv2d(dim, dim, kernel_size=1), nn.Conv2d(dim, dim, kernel_size=3, padding=1),
                                   nn.BatchNorm2d(dim), nn.ReLU(inplace=True))
    ...
    def forward(self, x, mask):
        ...
        mask_d = torch.sigmoid(mask_d)
        mask_d_b = 1 - mask_d
        xf = self.F_TA(x, mask_d_b)
        return xf

    def initialize(self):
        weight_init(self)

class MSASCD_SSM_module(nn.Module):
    def __init__(self, dim=128):
        super(MSASCD_SSM_module, self).__init__()
        self.F_TA = SSM_head()
        self.Fuse = nn.Conv2d(dim, dim, kernel_size=3, padding=1)
        self.Fuse2 = nn.Sequential(nn.Conv2d(dim, dim, kernel_size=1), nn.Conv2d(dim, dim, kernel_size=3, padding=1),
                                   nn.BatchNorm2d(dim), nn.ReLU(inplace=True))
