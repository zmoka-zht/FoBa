import sys
sys.path.append('/mnt/user/zhang_haotian/CDMamba')
        ...
        for i in range(len(down_x1)):
            ...
            if self.diff_abs == "later":
                if self.mode == "AGLGF":
                    ...
                    if i < self.stage:
                        x1, x2 = self.l_gf[i](x1, x2)
                elif self.mode == "GGF":
                    if i < self.stage:
                        x1, x2 = self.g_gf[i](x1, x2)
...
class CDMambaClass(nn.Module):

    def __init__(
            self,
            spitial_reslution: int = 224,
            spatial_dims: int = 3,
            init_filters: int = 16,
            in_channels: int = 1,
            out_channels: int = 2,
        ...
        self.resdiual = resdiual
        self.conv_mode = conv_mode
        self.spitial_reslution = spitial_reslution
        self.norm_reslution = int(self.spitial_reslution / 8)
        self.spatial_dims = spatial_dims
        self.init_filters = init_filters
        self.channels_list = [self.init_filters, self.init_filters*2, self.init_filters*4, self.init_filters*8]
        self.in_channels = in_channels
        ...
        print(self.norm)
        self.use_conv_final = use_conv_final
        # self.convInit = get_conv_layer(spatial_dims, in_channels, init_filters, kernel_size=7, stride=4)
        self.convInit = get_conv_layer(spatial_dims, in_channels, init_filters)
        self.srcm_encoder_layers = self._make_srcm_encoder_layers()
        self.classifier = nn.Sequential(OrderedDict(
            norm=nn.LayerNorm([self.channels_list[-1], self.norm_reslution, self.norm_reslution]),  # channels 16, 32, 64, 128
            avgpool=nn.AdaptiveAvgPool2d(1),
            flatten=nn.Flatten(1),
            head=nn.Linear(self.channels_list[-1], out_channels),
        ))
...
class CDMambaImageNet1k(nn.Module):
    ...
    def __init__(
        ...
        self.srcm_decoder_layers, self.up_samples = self._make_srcm_decoder_layers(up_mode=self.up_mode)
        self.conv_final = self._make_final_conv(out_channels)
        self.conv_final_scd1 = self._make_final_conv(scd_channels)
        self.conv_final_scd2 = self._make_final_conv(scd_channels)
        ...
        self.ag4 = AdaptiveGate(self.channels_list[3])
        self.ag = nn.Sequential(self.ag1, self.ag2, self.ag3, self.ag4)
        # ImageNet1k-train-CDMamba_32_241206_075943 ImageNet1k-train-CDMamba-16_241206_073747
        self.ckpt = torch.load(self.pretrain_path)
        self.cdmambaclass = CDMambaClass(spitial_reslution=256, spatial_dims=2, in_channels=3,
                                         out_channels=1000, init_filters=self.init_filters,
                                         norm=("GROUP", {"num_groups": 8}),
                                         conv_mode='orignal_dinner', stage=2, mamba_act="silu",
                                         blocks_down=(1, 2, 2, 4),
                                         resdiual=False)
        self.cdmambaclass.load_state_dict(self.ckpt, strict=True)
        self.encode = self.cdmambaclass.encode
    ...
    def decode(self, x: torch.Tensor, down_x: list[torch.Tensor]) -> torch.Tensor:
        ...
        if self.use_conv_final:
            x_bcd = self.conv_final(x)
            x_scd1 = self.conv_final_scd1(x)
            x_scd2 = self.conv_final_scd2(x)
        return x_bcd, x_scd1, x_scd2
    ...
    def forward(self, x1: torch.Tensor, x2:torch.Tensor) -> torch.Tensor:
        ...
        x_bcd, x_scd1, x_scd2 = self.decode(down_x[0], down_x)
        return x_bcd, x_scd1, x_scd2
...
if __name__ == "__main__":
    ...
    device = "cuda:0"
    # CDMamba = CDMambaClass(spitial_reslution=256, spatial_dims=2, in_channels=3,
    #                    out_channels=1000, init_filters=16,
    #                    norm=("GROUP", {"num_groups": 8}),
    #                    conv_mode='orignal_dinner', stage=2, mamba_act="silu",
    #                    blocks_down=(1, 2, 2, 4),
    #                    resdiual=False).to(device)
    CDMamba = CDMambaImageNet1k(spatial_dims=2, in_channels=3, out_channels=2, init_filters=16, norm=("GROUP", {"num_groups": 8}),
                    mode="AGLGF", conv_mode='orignal', local_query_model="orignal_dinner",
                    stage=2, mamba_act="silu", up_mode="SRCM", up_conv_mode='deepwise', blocks_down=(1, 2, 2, 4), blocks_up=(1, 1, 1),
                    resdiual=False, diff_abs="later",
                    pretrain_path="/mnt/user/zhang_haotian/CDMamba/experiments/ImageNet1k-train-CDMamba-16_241206_073747/checkpoint/best_cd_model_gen.pth").to(device)
    ...
    # print(cd_model)
    res, res1, res2 = CDMamba(img, img)
    print(res.shape)
    print(res1.shape)
    print(res2.shape)

    # flops1, params1 = profile(CDMamba, inputs=(img,))
    # print("flops=G", flops1 / (1000 ** 3))
    # print("parms=M", params1 / (1000 ** 2))5
    # param_num = 0
    # for name, param in CDMamba.named_parameters():
