...
from classification.models.vmamba import VSSM, LayerNorm2d, VSSBlock, Permute, VSSBlockMaskGuided, \
    VSSBlockMaskGuidedBackground
from changedetection.maskguided_utils.mask_guided_model import MSASCD_module, MSASCD_background_module
from changedetection.models.GIF import GIF
from changedetection.models.dysample import DySample

from changedetection.models.mamba_customer import Mamba_Fusion


def orthogonality_loss(fg_feat, bg_feat):
    # Flatten: [B, C, H, W] → [B, C, H*W]
    B, C, H, W = fg_feat.shape
    fg_flat = fg_feat.view(B, C, -1)
    bg_flat = bg_feat.view(B, C, -1)

    # Normalize along spatial dimension
    fg_norm = F.normalize(fg_flat, dim=2)
    bg_norm = F.normalize(bg_flat, dim=2)

    # Compute batch-wise inner product → [B]
    dot_product = torch.bmm(fg_norm, bg_norm.transpose(1, 2))  # [B, C, C]
    ortho = torch.mean(torch.abs(dot_product))  # 趋近于 0 为正交
...
class BaseChangeDecoder(nn.Module):
    def __init__(self, encoder_dims, channel_first, norm_layer, ssm_act_layer, mlp_act_layer, **kwargs):
        super(BaseChangeDecoder, self).__init__()
    ...
    def forward(self, pre_features, post_features):
        ...
        '''
            Stage I
        '''
        p4 = self.st_block_41(torch.cat([pre_feat_4, post_feat_4], dim=1))


        '''
            Stage II
        '''
        p3 = self.st_block_31(torch.cat([pre_feat_3, post_feat_3], dim=1))
        p3 = self._upsample_add(p4, p3)
        p3 = self.smooth_layer_3(p3)
        ...
        '''
            Stage III
        '''
        p2 = self.st_block_21(torch.cat([pre_feat_2, post_feat_2], dim=1))
        p2 = self._upsample_add(p3, p2)
        p2 = self.smooth_layer_2(p2)
        ...
        '''
            Stage IV
        '''
        p1 = self.st_block_11(torch.cat([pre_feat_1, post_feat_1], dim=1))
        p1 = self._upsample_add(p2, p1)
        p1 = self.smooth_layer_1(p1)
...
class FoBaDecoderSSM(nn.Module):
    def __init__(self, encoder_dims, channel_first, norm_layer, ssm_act_layer, mlp_act_layer, **kwargs):
        super(FoBaDecoderSSM, self).__init__()
        ...
        ######### foreground mask generate #########
        self.fmg2 = nn.Conv2d(128, 1, kernel_size=3, padding=1)
        self.fmg3 = nn.Conv2d(128, 1, kernel_size=3, padding=1)
        self.fmg4 = nn.Conv2d(128, 1, kernel_size=3, padding=1)


        self.fgm_ssm3 = VSSBlockMaskGuided(hidden_dim=128, drop_path=0.1, norm_layer=norm_layer, channel_first=channel_first,
                     ssm_d_state=kwargs['ssm_d_state'], ssm_ratio=kwargs['ssm_ratio'],
                     ssm_dt_rank=kwargs['ssm_dt_rank'], ssm_act_layer=ssm_act_layer,
                     ssm_conv=kwargs['ssm_conv'], ssm_conv_bias=kwargs['ssm_conv_bias'],
                     ssm_drop_rate=kwargs['ssm_drop_rate'], ssm_init=kwargs['ssm_init'],
        ...
        self.fgm_ssm2 = VSSBlockMaskGuided(hidden_dim=128, drop_path=0.1, norm_layer=norm_layer,
                                           ...
                                           mlp_ratio=kwargs['mlp_ratio'], mlp_act_layer=mlp_act_layer,
                                           mlp_drop_rate=kwargs['mlp_drop_rate'],
                                           gmlp=kwargs['gmlp'], use_checkpoint=kwargs['use_checkpoint'])

        self.fgm_ssm1 = VSSBlockMaskGuided(hidden_dim=128, drop_path=0.1, norm_layer=norm_layer,
                                           channel_first=channel_first,
                                           ssm_d_state=kwargs['ssm_d_state'], ssm_ratio=kwargs['ssm_ratio'],
                                           ssm_dt_rank=kwargs['ssm_dt_rank'], ssm_act_layer=ssm_act_layer,
                                           ssm_conv=kwargs['ssm_conv'],
        ...
        self.bgm_ssm3 = VSSBlockMaskGuidedBackground(hidden_dim=128, drop_path=0.1, norm_layer=norm_layer,
                                                   channel_first=channel_first,
                                                   ssm_d_state=kwargs['ssm_d_state'], ssm_ratio=kwargs['ssm_ratio'],
                                                   ssm_dt_rank=kwargs['ssm_dt_rank'], ssm_act_layer=ssm_act_layer,
                                                   ssm_conv=kwargs['ssm_conv'],
        ...
        self.bgm_ssm2 = VSSBlockMaskGuidedBackground(hidden_dim=128, drop_path=0.1, norm_layer=norm_layer,
                                                   channel_first=channel_first,
                                                   ssm_d_state=kwargs['ssm_d_state'], ssm_ratio=kwargs['ssm_ratio'],
                                                   ssm_dt_rank=kwargs['ssm_dt_rank'], ssm_act_layer=ssm_act_layer,
                                                   ssm_conv=kwargs['ssm_conv'],
        ...
        self.bgm_ssm1 = VSSBlockMaskGuidedBackground(hidden_dim=128, drop_path=0.1, norm_layer=norm_layer,
                                                   channel_first=channel_first,
                                                   ssm_d_state=kwargs['ssm_d_state'], ssm_ratio=kwargs['ssm_ratio'],
                                                   ssm_dt_rank=kwargs['ssm_dt_rank'], ssm_act_layer=ssm_act_layer,
                                                   ssm_conv=kwargs['ssm_conv'],
                                                   ...
                                                   gmlp=kwargs['gmlp'], use_checkpoint=kwargs['use_checkpoint'])
        #################GIF#################
        self.gif_41 = GIF(in_channels=encoder_dims[-1])
        self.gif_42 = GIF(in_channels=encoder_dims[-1])
        self.gif_31 = GIF(in_channels=encoder_dims[-2])
        self.gif_32 = GIF(in_channels=encoder_dims[-2])
        self.gif_21 = GIF(in_channels=encoder_dims[-3])
        self.gif_22 = GIF(in_channels=encoder_dims[-3])
        self.gif_11 = GIF(in_channels=encoder_dims[-4])
        self.gif_12 = GIF(in_channels=encoder_dims[-4])
    ...
    def forward(self, pre_features, post_features):
        ...
        pre_feat_4 = self.gif_41(pre_feat_4, post_feat_4)
        post_feat_4 = self.gif_42(post_feat_4, pre_feat_4)
        p4 = self.st_block_41(torch.cat([pre_feat_4, post_feat_4], dim=1))
        pred_mask_4 = self.fmg4(p4)

        '''
            Stage II
        '''
        pre_feat_3 = self.gif_31(pre_feat_3, post_feat_3)
        post_feat_3 = self.gif_32(post_feat_3, pre_feat_3)
        p3 = self.st_block_31(torch.cat([pre_feat_3, post_feat_3], dim=1))

        # Mask guided
        guided_p3 = self.fgm_ssm3(p3, pred_mask_4)
        guided_p3_background = self.bgm_ssm3(p3, pred_mask_4)
        guided_p3 = guided_p3 + guided_p3_background

        p3 = self._dyupsample_add_3(p4, guided_p3)
        p3 = self.smooth_layer_3(p3)
        pred_mask_3 = self.fmg3(p3)

        '''
            Stage III
        '''
        pre_feat_2 = self.gif_21(pre_feat_2, post_feat_2)
        post_feat_2 = self.gif_22(post_feat_2, pre_feat_2)
        p2 = self.st_block_21(torch.cat([pre_feat_2, post_feat_2], dim=1))

        # Mask guided
        guided_p2 = self.fgm_ssm2(p2, pred_mask_3)
        guided_p2_background = self.bgm_ssm2(p2, pred_mask_3)
        guided_p2 = guided_p2 + guided_p2_background

        p2 = self._dyupsample_add_2(p3, guided_p2)
        p2 = self.smooth_layer_2(p2)
        pred_mask_2 = self.fmg2(p2)

        '''
            Stage IV
        '''
        pre_feat_1 = self.gif_11(pre_feat_1, post_feat_1)
        post_feat_1 = self.gif_12(post_feat_1, pre_feat_1)
        p1 = self.st_block_11(torch.cat([pre_feat_1, post_feat_1], dim=1))

        # Mask guided
        guided_p1 = self.fgm_ssm1(p1, pred_mask_2)
        guided_p1_background = self.bgm_ssm1(p1, pred_mask_2)
        guided_p1 = guided_p1 + guided_p1_background

        p1 = self._dyupsample_add_1(p2, guided_p1)
        p1 = self.smooth_layer_1(p1)

        return p1, pred_mask_2, pred_mask_3, pred_mask_4

class FoBaDecoderTransformer(nn.Module):
    def __init__(self, encoder_dims, channel_first, norm_layer, ssm_act_layer, mlp_act_layer, **kwargs):
        super(FoBaDecoderTransformer, self).__init__()
        ...
        #####################
        self.fgm_transformer3 = MSASCD_module(dim=128)
        self.fgm_transformer2 = MSASCD_module(dim=128)
        self.fgm_transformer1 = MSASCD_module(dim=128)
        self.bgm_transformer3 = MSASCD_background_module(dim=128)
        self.bgm_transformer2 = MSASCD_background_module(dim=128)
        self.bgm_transformer1 = MSASCD_background_module(dim=128)
    ...
    def forward(self, pre_features, post_features):
        ...
        guided_p3 = self.fgm_transformer3(p3, pred_mask_4)
        guided_p3_background = self.bgm_transformer3(p3, pred_mask_4)
        guided_p3 = guided_p3 + guided_p3_background
        p3 = self._upsample_add(p4, guided_p3)
        ...
        p2 = self.st_block_21(torch.cat([pre_feat_2, post_feat_2], dim=1))
        # Mask guided
        guided_p2 = self.fgm_transformer2(p2, pred_mask_3)
        guided_p2_background = self.bgm_transformer2(p2, pred_mask_3)
        guided_p2 = guided_p2 + guided_p2_background
        p2 = self._upsample_add(p3, guided_p2)
        ...
        p1 = self.st_block_11(torch.cat([pre_feat_1, post_feat_1], dim=1))
        # Mask guided
        guided_p1 = self.fgm_transformer1(p1, pred_mask_2)
        guided_p1_background = self.bgm_transformer1(p1, pred_mask_2)
        guided_p1 = guided_p1 + guided_p1_background
        p1 = self._upsample_add(p2, guided_p1)
        p1 = self.smooth_layer_1(p1)
