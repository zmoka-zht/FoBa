...
class VSSBlockMaskGuided(nn.Module):
    def __init__(
            self,
            hidden_dim: int = 0,
            drop_path: float = 0,
    ...
    def _forward(self, input: torch.Tensor, mask):
        # import pdb; pdb.set_trace()
        input = input.permute(0, 2, 3, 1) # (B, C, H, W)-->(B, H, W, C)
        mask = F.interpolate(mask, size=input.size()[1:3], mode='bilinear')
        mask_d = mask.detach()
        mask_d = torch.sigmoid(mask_d)
        mask_d = mask_d.permute(0, 2, 3, 1)
        if self.ssm_branch:
            if self.post_norm:
                x = input + self.drop_path(self.norm(self.op(input * mask_d)))
            else:
                x = input + self.drop_path(self.op(self.norm(input) * mask_d))
        if self.mlp_branch:
            if self.post_norm:
                x = x + self.drop_path(self.norm2(self.mlp(x)))  # FFN
            else:
...
class VSSBlockMaskGuidedBackground(nn.Module):
    def __init__(
            self,
            hidden_dim: int = 0,
            drop_path: float = 0,
    ...
    def _forward(self, input: torch.Tensor, mask):
        ...
        mask = F.interpolate(mask, size=input.size()[1:3], mode='bilinear')
        mask_d = mask.detach()
        mask_d_b = 1 - torch.sigmoid(mask_d)
        mask_d_b = mask_d_b.permute(0, 2, 3, 1)
        if self.ssm_branch:
            if self.post_norm:
                x = input + self.drop_path(self.norm(self.op(input * mask_d_b)))
            else:
                x = input + self.drop_path(self.op(self.norm(input) * mask_d_b))
        if self.mlp_branch:
            if self.post_norm:
                x = x + self.drop_path(self.norm2(self.mlp(x)))  # FFN
            else:
